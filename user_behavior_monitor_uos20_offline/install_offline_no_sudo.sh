#!/bin/bash
# UOS20用户行为监控系统离线安装脚本 - 无sudo版
# 版本: v1.3.0 离线版
# 生成时间: 2025-07-29 11:20:00
...
check_system() {
    ...
    if ! command -v pip3 &> /dev/null; then
        log_warn "未找到pip3，尝试使用python3 -m pip..."
        if python3 -m pip --version &> /dev/null; then
            log_info "找到python3 -m pip，将使用此方式安装包"
        else
            log_error "未找到pip3或python3 -m pip"
            log_info "请手动安装pip3或联系系统管理员"
            exit 1
        fi
    else
...
check_system_deps() {
    ...
    if [[ ${#missing_packages[@]} -gt 0 ]]; then
        log_warn "检测到缺失的系统依赖: ${missing_packages[*]}"
        log_info "由于没有sudo权限，请手动安装这些包:"
        log_info "  Ubuntu/Debian: sudo apt install ${missing_packages[*]}"
        log_info "  CentOS/RHEL: sudo yum install ${missing_packages[*]}"
        log_info "  Fedora: sudo dnf install ${missing_packages[*]}"
        log_info "或者联系系统管理员安装这些包"
        log_warn "是否继续安装? (y/N)"
        read -r response
...
# 离线安装Python依赖 - 无sudo版
install_python_deps_offline_no_sudo() {
    log_step "离线安装Python依赖 (无sudo版)..."
    ...
    if [[ -d "$wheels_dir" && "$(ls -A "$wheels_dir")" ]]; then
        ...
        for package_file in "$wheels_dir"/*; do
            if [[ -f "$package_file" ]]; then
                ...
                if command -v pip3 &> /dev/null; then
                    pip3 install "$package_file" --no-deps
                else
                    python3 -m pip install "$package_file" --no-deps
                fi
            fi
        done
        ...
        log_info "检查并安装缺失的依赖..."
        if command -v pip3 &> /dev/null; then
            pip3 install -r requirements_uos20.txt --no-deps || true
        else
            python3 -m pip install -r requirements_uos20.txt --no-deps || true
        fi
        
    else
        log_warn "未发现离线Python包，尝试在线安装..."
        install_python_deps_online_no_sudo
    fi
}

# 在线安装Python依赖 - 无sudo版
install_python_deps_online_no_sudo() {
    log_info "在线安装Python依赖 (无sudo版)..."
    ...
    if command -v pip3 &> /dev/null; then
        ...
        python3 -m pip install -r requirements_uos20.txt
    fi
    
    # 安装额外依赖
    log_info "安装额外依赖..."
    if command -v pip3 &> /dev/null; then
        pip3 install python-xlib python-evdev setuptools wheel
    else
        python3 -m pip install python-xlib python-evdev setuptools wheel
    fi
...
set_permissions() {
    ...
    chmod +x install_offline_no_sudo.sh
...
# 检查服务状态（如果有权限）
echo "系统服务状态:"
sudo python3 uos20_service_manager.py status 2>/dev/null || echo "服务未安装或无权限"
...
try:
    ...
    print('将尝试在线安装缺失的包...')
    import subprocess
    subprocess.run(['python3', '-m', 'pip', 'install', 'numpy', 'pandas', 'scikit-learn', 'xgboost', 'psutil', 'pynput', 'keyboard', 'pyyaml'])
"
...
# 主安装流程
main() {
    echo "开始离线安装UOS20用户行为监控系统 (无sudo版)..."
    echo ""
